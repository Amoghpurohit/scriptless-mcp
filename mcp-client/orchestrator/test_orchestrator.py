...
class TestOrchestrator:
    ...
    def __init__(self, 
        ...
        self.prev_page_state: Optional[str] = None  # Store previous browser page state (for diffs)
                    ...
                    if content.get('type') == 'text':
                        ...
                        if 'Page Snapshot:' in text and 'ref=' in text:
                            ...
                            if yaml_start > -1 and yaml_end > -1:
                                ...
                                # Shift last to prev, then update last
                                self.prev_page_state = self.last_page_state
    ...
    async def __aenter__(self):
        ...
        self.mcp_client = await create_mcp_http_client(self.mcp_server_url, browser_args=self.browser_args)
    ...
    async def _execute_test_step(self, 
        ...
        try:
            with Progress(
                ...
                # Use compact DOM context if available
                compact_state = None
                try:
                    compact_state = self.context_manager.get_minimal_dom_context(max_lines=80)
                except Exception:
                    compact_state = self.last_page_state
                test_step = await self.llm_service.parse_natural_language_step(
                    nl_prompt, available_tools, compact_state
                )
                # If compact context misses the target element/ref, fallback to full cached snapshot
                try:
                    need_fallback = False
                    if isinstance(test_step.parameters, dict):
                        ref = test_step.parameters.get("ref")
                        element = test_step.parameters.get("element")
                        minimal = compact_state or ""
                        if ref and minimal and (ref not in minimal):
                            need_fallback = True
                        elif element and minimal and (element not in minimal):
                            need_fallback = True
                    if need_fallback:
                        full_state = self.context_manager.get_full_dom_snapshot(max_chars=10000)
                        test_step = await self.llm_service.parse_natural_language_step(
                            nl_prompt, available_tools, full_state
                        )
                except Exception:
                    pass
                if self.session_recorder:
                ...
                try:
                    # Enrich default params for screenshot tool
                    if test_step.action_type == "browser_take_screenshot":
                        output_dir = Path(__file__).parent.parent / "output"
                        output_dir.mkdir(parents=True, exist_ok=True)
                        default_filename = output_dir / f"{self.context_manager.current_session.session_id}_step_{self.current_step:03d}.png"
                        if not isinstance(test_step.parameters, dict):
                            test_step.parameters = {}
                        test_step.parameters.setdefault("raw", True)
                        test_step.parameters.setdefault("fullPage", False)
                        test_step.parameters.setdefault("filename", str(default_filename))
                    ...
                    if self.session_recorder:
                        try:
                            ...
                            pass
                    # Let the client own the SSE wait; pass step timeout into client to avoid nested wait_for cancellation issues
                    mcp_response = await self.mcp_client.call_tool(
                        test_step.action_type,
                        test_step.parameters,
                        timeout=self.step_timeout,
                    )
                    # Prepare snapshot source defaults (use primary tool result unless we capture a fresh browser_snapshot)
                    snapshot_source_tool_name = test_step.action_type
                    snapshot_source_params = test_step.parameters
                    snapshot_source_result = mcp_response

                    # If we only got an accepted status (no payload), try a quick snapshot/wait to surface UI changes
                    if isinstance(mcp_response, dict) and mcp_response.get("status") == "accepted":
                        try:
                            # Small settle delay for navigation/UI updates
                            await asyncio.sleep(0.3)
                            if self.mcp_client.has_tool("browser_snapshot"):
                                # Try up to 2 times to obtain a textual snapshot
                                for _attempt in range(2):
                                    snap = await self.mcp_client.call_tool("browser_snapshot", {}, timeout=min(self.step_timeout, 5))
                                    # If snapshot contains content, prefer it as the snapshot source
                                    if isinstance(snap, dict) and (snap.get("content") or snap.get("result", {}).get("content")):
                                        snapshot_source_tool_name = "browser_snapshot"
                                        snapshot_source_params = {}
                                        snapshot_source_result = snap
                                        break
                                    await asyncio.sleep(0.3)
                            elif self.mcp_client.has_tool("browser_wait_for") and isinstance(test_step.parameters, dict):
                                wait_args = {}
                                if test_step.parameters.get("element"):
                                    wait_args = {"text": str(test_step.parameters["element"]), "time": 1}
                                if wait_args:
                                    _ = await self.mcp_client.call_tool("browser_wait_for", wait_args, timeout=min(self.step_timeout, 5))
                        except Exception:
                            pass
                    ...
                    if hasattr(test_step, 'additional_parameters') and test_step.additional_parameters:
                        ...
                        for i, additional_params in enumerate(test_step.additional_parameters):
                            ...
                            additional_response = await self.mcp_client.call_tool(
                                test_step.action_type,
                                additional_params,
                                timeout=self.step_timeout,
                            )
                    ...
                    snapshot_data = await self.mcp_client.get_snapshot_data(
                        snapshot_source_tool_name, snapshot_source_params, snapshot_source_result
                    )
                    if self.session_recorder:
                        try:
                    ...
                    # Update DOM cache for prompt compaction
                    page_url = None
                    page_title = None
                    snapshot_text = None
                    try:
                        # Try extract URL/Title from textual content
                        if isinstance(mcp_response, dict):
                            for entry in mcp_response.get("content", []) or []:
                                if isinstance(entry, dict) and entry.get("type") == "text":
                                    txt = entry.get("text", "")
                                    if "Page URL:" in txt:
                                        page_url = txt.split("Page URL:", 1)[1].split("\n", 1)[0].strip()
                                    if "Page Title:" in txt:
                                        page_title = txt.split("Page Title:", 1)[1].split("\n", 1)[0].strip()
                        # Prefer snapshot from snapshot_data if present
                        if isinstance(snapshot_data, dict):
                            tool_result = snapshot_data.get("tool_result", {})
                            for entry in tool_result.get("content", []) or []:
                                if isinstance(entry, dict) and entry.get("type") == "text":
                                    text_block = entry.get("text", "")
                                    if "Page Snapshot:" in text_block and "ref=" in text_block:
                                        snapshot_text = text_block
                                        # Also derive URL/Title if missing
                                        if (not page_url) and "Page URL:" in text_block:
                                            page_url = text_block.split("Page URL:", 1)[1].split("\n", 1)[0].strip()
                                        if (not page_title) and "Page Title:" in text_block:
                                            page_title = text_block.split("Page Title:", 1)[1].split("\n", 1)[0].strip()
                                    # Fallback: if a dialog/modal section is present without explicit header
                                    if snapshot_text is None and ("dialog" in text_block or "modal" in text_block or "overlay" in text_block):
                                        snapshot_text = text_block
                        self.context_manager.update_dom_cache(page_url, page_title, snapshot_text)
                    except Exception:
                        pass
                ...
                # Compute UI change signals to help LLM reason about action effects
                def _count_refs(s: Optional[str]) -> int:
                    try:
                        return s.count('ref=') if s else 0
                    except Exception:
                        return 0
                prev_state = self.prev_page_state
                curr_state = self.last_page_state
                prev_refs = _count_refs(prev_state)
                curr_refs = _count_refs(curr_state)
                snapshot_changed = bool(curr_state and (curr_state != (prev_state or "")))
                new_keywords = []
                try:
                    kw_list = [
                        'dialog', 'modal', 'overlay', 'combobox', 'textbox', 'button',
                        'Propose Swap', 'Send Proposal', 'My Swaps', 'Messages', 'Create a Listing'
                    ]
                    lower_prev = (prev_state or '').lower()
                    lower_curr = (curr_state or '').lower()
                    for kw in kw_list:
                        if kw.lower() in lower_curr and kw.lower() not in lower_prev:
                            new_keywords.append(kw)
                except Exception:
                    pass
                # Build a small diff summary (counts and representative added lines)
                added_lines_count = 0
                removed_lines_count = 0
                added_line_samples: list[str] = []
                try:
                    import difflib
                    prev_lines = (prev_state or "").splitlines()
                    curr_lines = (curr_state or "").splitlines()
                    for line in difflib.unified_diff(prev_lines, curr_lines, lineterm=""):
                        ...
                        if line.startswith("+"):
                            ...
                            text = line[1:].strip()
                            ltext = text.lower()
                            if len(added_line_samples) < 10 and (
                                "ref=" in text or "button" in ltext or "dialog" in ltext or "modal" in ltext or "combobox" in ltext
                            ):
                                added_line_samples.append(text[:200])
                        elif line.startswith("-"):
                            removed_lines_count += 1
                except Exception:
                    pass

                # Expected element/ref presence
                expected_label = None
                expected_ref = None
                try:
                    if isinstance(test_step.parameters, dict):
                        el = test_step.parameters.get("element")
                        rf = test_step.parameters.get("ref")
                        expected_label = str(el) if el is not None else None
                        expected_ref = str(rf) if rf is not None else None
                except Exception:
                    pass
                lower_curr_state = (curr_state or "").lower()
                expected_label_present = bool(expected_label) and (expected_label.lower() in lower_curr_state)
                expected_ref_present = bool(expected_ref) and (expected_ref in (curr_state or ""))
                require_element_evidence = test_step.action_type in {
                    "browser_click", "browser_type", "browser_select_option", "browser_hover", "browser_drag", "browser_press_key"
                }
                element_evidence_present = expected_label_present or expected_ref_present

                analysis_hints = {
                    "new_snapshot_received": bool(curr_state),
                    "snapshot_changed": snapshot_changed,
                    "previous_refs_count": prev_refs,
                    "current_refs_count": curr_refs,
                    "new_ui_keywords_detected": new_keywords,
                    "diff_summary": {
                        "added_lines_count": added_lines_count,
                        "removed_lines_count": removed_lines_count,
                        "added_line_samples": added_line_samples,
                    },
                    "expected_element_label": expected_label,
                    "expected_element_ref": expected_ref,
                    "expected_element_label_present": expected_label_present,
                    "expected_element_ref_present": expected_ref_present,
                    "require_element_evidence": require_element_evidence,
                    "element_evidence_present": element_evidence_present,
                    "page_url": page_url,
                    "page_title": page_title,
                }
                llm_analysis = await self.llm_service.analyze_step_result(
                    test_step, mcp_response, snapshot_data, analysis_hints
                )
                progress.update(task4, completed=True)

                # If step failed per LLM analysis, screenshot capture is disabled by request
                if llm_analysis.pass_fail is False:
                    logger.info("Skipping failure screenshot: screenshot tool disabled for test runs")
            ...
            # Screenshot on unexpected failure is disabled by request
