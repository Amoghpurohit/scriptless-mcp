...
class MCPHTTPClient:
    ...
    def __init__(self, server_url: str, timeout: int = 30, recorder: Optional[SessionLogRecorder] = None):
            ...
            async for line in self.sse_connection.content:
                ...
                if line.startswith('event: endpoint'):
                    ...
                    if 'sessionId=' in endpoint_data:
                        # Extract session ID from URL like "/sse?sessionId=xxx"
                        ...
                        # Handle other SSE events (server messages, JSON-RPC responses, etc.)
                        logger.debug(f"Received SSE data: {endpoint_data}")
                        if self.recorder:
                            try:
                                self.recorder.record_sse_event(endpoint_data)
                            except Exception:
                                pass
                        
                        # Try to parse as JSON-RPC response
                        try:
                            response_data = json.loads(endpoint_data)
                            ...
                            elif 'result' in response_data and 'tools' in response_data.get('result', {}):
                            ...
                            # Not JSON, just log as regular SSE data
                            pass
    ...
    async def call_tool(self, tool_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        execution_future = asyncio.Future()
...
async def create_mcp_http_client(server_url: str, timeout: int = 30) -> MCPHTTPClient:
    """Create and initialize MCP HTTP client"""
    client = MCPHTTPClient(server_url, timeout)
